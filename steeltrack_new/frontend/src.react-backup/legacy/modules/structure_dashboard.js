...
const structures = state.data.structures || [];
    ...
    const stockStats = structures.map(s => ({
        ...
        totalValue: (Number(s.qty) || 0) * (Number(s.cost) || 0)
    })).sort((a, b) => b.totalValue - a.totalValue);
    
    // Lọc giao dịch sản xuất (produce) - KHÔNG phân biệt chữ hoa/thường
    const produceTxns = transactions.filter(t => t.type && t.type.toLowerCase() === 'produce');
    const exportTxns = transactions.filter(t => t.type && t.type.toLowerCase() === 'structure_export');
    ...
    const totalExportValue = exportTxns.reduce((sum, t) => sum + (Number(t.totalAmount) || 0), 0);
    ...
    produceTxns.forEach(t => {
        ...
        if (structure) {
            ...
            if (!productionStats[name]) {
                productionStats[name] = { name: name, unit: structure.unit, qty: 0 };
            }
            productionStats[name].qty += Number(t.qty) || 0;
        }
    });
    const topProduced = Object.values(productionStats).sort((a, b) => b.qty - a.qty).slice(0, 5);
    
    // Sản xuất theo tháng (6 tháng)
    ...
    produceTxns.forEach(t => {
        ...
        if (monthlyProduction[key]) {
            monthlyProduction[key].produce += Number(t.qty) || 0;
        }
    });
    ...
    exportTxns.forEach(t => {
        ...
        const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
        if (monthlyProduction[key]) {
            monthlyProduction[key].export += Number(t.qty) || 0;
        }
    });
    ...
    const result = {
        ...
        stockValue: stockStats.reduce((sum, s) => sum + s.totalValue, 0),
        lowStockCount: stockStats.filter(s => s.qty < 10).length
    };
    
    console.log('📊 Structure Stats:', result);
    return result;
}
// ========== RENDER KPI CARDS ==========
